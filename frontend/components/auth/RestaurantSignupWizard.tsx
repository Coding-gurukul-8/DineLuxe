...
type RestaurantSignupForm = z.infer<typeof restaurantSignupSchema>
...
const steps = [
  {
    ...
    description: "Tell us about yourself (the account owner)",
    fields: [
      { name: "firstName" as const, label: "First Name", placeholder: "Jane", icon: <User size={18} /> },
      { name: "lastName"  as const, label: "Last Name",  placeholder: "Smith", icon: <User size={18} /> },
      { name: "email"     as const, label: "Email",      placeholder: "jane@restaurant.com", type: "email", icon: <Mail size={18} /> },
      { name: "phone"     as const, label: "Phone",      placeholder: "+91 98765 43210", type: "tel", icon: <Phone size={18} /> },
    ],
  },
  {
    ...
    fields: [
      { name: "password"        as const, label: "Password",         placeholder: "Create a strong password",  type: "password", icon: <Lock size={18} /> },
      { name: "confirmPassword" as const, label: "Confirm Password", placeholder: "Re-enter your password",    type: "password", icon: <Lock size={18} /> },
    ],
  },
]
...
export function RestaurantSignupWizard() {
  ...
  const handleNext = async () => {
    ...
    if (isValid) { setDirection(1); setStep((p) => p + 1); setError(null) }
  }

  const handleBack = () => { setDirection(-1); setStep((p) => p - 1); setError(null) }
  ...
  const onSubmit = async (data: RestaurantSignupForm) => {
    ...
    try {
      ...
      if (result.verification_pending) {
        setTimeout(() => {
          router.push(`/auth/verify-otp?email=${encodeURIComponent(data.email)}&portal=restaurant`)
        }, 1500)
      } else {
          ...
          {isLastStep ? (
            ...
            >
              {isSubmitting ? (
                ...
                <><Store size={18} className="mr-1" />Create Account</>
              )}
            </Button>
          ) : (
            <Button
