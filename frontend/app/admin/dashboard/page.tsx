  ...
  return (
    <div
      ...
      interface PlatformStats {
        ...
        total_revenue_today: number;
        platform_health_score?: number;
        top_restaurants?: { id: string; name: string; revenue: number; orders: number; status?: string }[];
      }
      ...
      function RestaurantStatusGrid({ restaurants }: { restaurants: PlatformStats["top_restaurants"] }) {
        ...
        const items = (restaurants ?? []).filter((r: any) => {
        ...
        return (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {items.length === 0 && <p className="col-span-2 text-xs text-gray-400 text-center py-4">No data</p>}
            </div>
          </div>
        );
      }
      ...
      export default function AdminDashboardPage() {
        ...
        const sparkOrders = growthData.slice(-7).map((d) => d.orders);
        const sparkRevenue = growthData.slice(-7).map((d) => d.revenue);
              ...
              {isLoading
                ...
                ? (
                    <div className="col-span-4 flex flex-col items-center py-12 gap-3 text-gray-400">
                      ...
                      <button onClick={() => refetch()} className="text-sm text-[#1A3C5E] hover:underline flex items-center gap-1">
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={growthData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                    ...
                    <Line type="monotone" dataKey="orders" stroke="#1A3C5E" strokeWidth={2} dot={false} isAnimationActive animationDuration={1200} animationEasing="ease-out" />
                    <Line type="monotone" dataKey="restaurants" stroke="#E8A020" strokeWidth={2} dot={false} isAnimationActive animationDuration={1200} animationEasing="ease-out" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
