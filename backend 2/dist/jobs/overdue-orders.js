...
exports.runOverdueOrdersCheck = runOverdueOrdersCheck;
...
async function runOverdueOrdersCheck() {
    const threshold = new Date(Date.now() - OVERDUE_THRESHOLD_MINUTES * 60 * 1000).toISOString();
    // FIX: removed 'overdue_alerted_at' filter — column does not exist in schema.
    // Use Redis to prevent duplicate alerts instead.
    const { data: overdueOrders, error } = await supabase_1.supabaseAdmin
        .from('orders')
    ...
    for (const order of overdueOrders) {
        try {
            ...
            await supabase_1.supabaseAdmin.channel(`kitchen:${order.branch_id}`).send({
                type: 'broadcast',
                event: 'overdue_order',
                payload: {
                    order_id: order.id,
            ...
            await supabase_1.supabaseAdmin.channel(`manager:${order.branch_id}`).send({
                type: 'broadcast',
                event: 'overdue_order',
                payload: {
                    order_id: order.id,
...
//# sourceMappingURL=overdue-orders.js.map