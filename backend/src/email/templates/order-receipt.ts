...
export function orderReceiptTemplate(
  ...
  const {
    customerName, restaurantName, restaurantLogo,
    items, subtotal, gst, serviceCharge, total,
    paymentMethod, orderId, date,
  } = data;
  ...
  const summaryRow = (label: string, value: string, bold = false, highlight = false) => `
    <tr>
