...
exports.runNoShowCancellation = runNoShowCancellation;
...
async function runNoShowCancellation() {
    const graceCutoff = new Date(Date.now() - NO_SHOW_GRACE_MINUTES * 60 * 1000).toISOString();
    // FIX: 'assigned_table_id' → 'table_id' (actual schema column name)
    const { data: noShows, error } = await supabase_1.supabaseAdmin
        .from('queue_entries')
    ...
    for (const entry of noShows) {
        try {
            ...
            await supabase_1.supabaseAdmin.channel(`host:${entry.branch_id}`).send({
                type: 'broadcast',
                event: 'queue_updated',
                payload: {
            ...
            if (entry.user_id) {
                (0, notifications_service_1.sendPush)(entry.user_id, 'Queue Position Removed', `Your queue spot was removed after ${NO_SHOW_GRACE_MINUTES} minutes. Please rejoin if you'd like a table.`, { type: 'queue_no_show', branch_id: entry.branch_id });
            }
...
//# sourceMappingURL=no-show-cancel.js.map