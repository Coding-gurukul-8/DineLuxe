...
async function getSales(params) {
    ...
    const { data, error } = await supabase_1.supabaseAdmin.rpc('get_sales_report', {
...
async function getMenuPerformance(restaurantId, branchId) {
    ...
    const { data, error } = await supabase_1.supabaseAdmin.rpc('get_menu_performance', {
...
// ─── Kitchen performance ──────────────────────────────────────────────────────
async function getKitchenPerformance(branchId, from, to) {
    const { data, error } = await supabase_1.supabaseAdmin.rpc('get_kitchen_performance', {
...
async function getCustomerInsights(restaurantId) {
    const [newCustomers, returning, topSpenders] = await Promise.all([
        ...
        supabase_1.supabaseAdmin.rpc('get_returning_customers', { p_restaurant_id: restaurantId }),
        supabase_1.supabaseAdmin.rpc('get_top_spenders', {
            p_restaurant_id: restaurantId,
            p_limit: 10,
        }),
    ]);
...
async function getAdminTrends(from, to) {
    const { data, error } = await supabase_1.supabaseAdmin.rpc('get_platform_trends', {
        p_from: from,
        p_to: to,
    });
    if (error) {
...
async function exportReport(params) {
    ...
    if (report_type === 'kitchen-performance' && !branch_id) {
        throw Object.assign(new Error('branch_id is required for kitchen-performance export'), { status: 422 });
    }
    ...
    if (report_type === 'sales') {
        ...
        reportData = [
            ...
            ...insights.top_spenders.map((s) => ({ metric: 'top_spender', ...s })),
        ];
    }
    ...
    const { data: urlData, error: urlError } = await supabase_1.supabaseAdmin.storage
        .from('exports')
