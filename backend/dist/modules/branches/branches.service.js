...
// ─── Get Single Branch ────────────────────────────────────────────────────────
async function getById(branchId, restaurantId) {
    const { data, error } = await supabase_1.supabaseAdmin
        .from('branches')
...
// ─── Update Branch ────────────────────────────────────────────────────────────
async function update(branchId, restaurantId, input) {
    ...
    const { data, error } = await supabase_1.supabaseAdmin
        ...
        .single();
    if (error)
        throw new Error(`Branch update failed: ${error.message}`);
    return data;
}
// ─── Toggle Status (open/close) ───────────────────────────────────────────────
// FIX: branch uses 'is_active' boolean, not a 'status' string column
async function toggleStatus(branchId, restaurantId, input, actorId, ipAddress) {
    ...
    const { data, error } = await supabase_1.supabaseAdmin
        ...
        .single();
    if (error)
        throw new Error(`Status toggle failed: ${error.message}`);
    await (0, audit_log_1.insertAuditLog)({
        actorId,
...
async function getLiveStats(branchId, restaurantId) {
    ...
    const tablesByStatus = tables.reduce((acc, t) => {
        acc[t.status] = (acc[t.status] ?? 0) + 1;
        return acc;
    }, {});
    const revenueToday = (itemsRes.data ?? []).reduce((sum, i) => sum + Number(i.unit_price) * Number(i.quantity), 0);
    return {
