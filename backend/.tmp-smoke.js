...
(async () => {
  await hit('health', 'GET', '/admin/health', null, false);
  await hit('admin-dashboard', 'GET', '/admin/dashboard');
  await hit('admin-platform-stats', 'GET', '/admin/platform-stats');
  await hit('admin-health-detailed', 'GET', '/admin/health/detailed');
  await hit('admin-restaurants', 'GET', '/admin/restaurants');
  await hit('admin-customers', 'GET', '/admin/customers');
  await hit('admin-feedback', 'GET', '/admin/feedback');

  await hit('users-me', 'GET', '/users/me');
  await hit('users-by-id', 'GET', `/users/e9b58c31-1c75-410a-8775-9387c9c16670`);

  await hit('restaurants-list', 'GET', '/restaurants');
  await hit('restaurants-get', 'GET', `/restaurants/${restaurantId}`, null, false);
  await hit('restaurants-live', 'GET', `/restaurants/${restaurantId}/live-status`, null, false);

  await hit('branches-list', 'GET', '/branches');
  await hit('branches-live-stats', 'GET', `/branches/${branchId}/live-stats`);

  await hit('branding-get', 'GET', `/restaurant/${restaurantId}/branding`, null, false);

  await hit('menu-public', 'GET', `/menu/branch/${branchId}`, null, false);
  await hit('menu-categories', 'GET', `/menu/branch/${branchId}/categories`);

  await hit('tables-list', 'GET', `/tables/branch/${branchId}`);

  await hit('floor-layout', 'GET', `/floor-layout/branch/${branchId}`);
