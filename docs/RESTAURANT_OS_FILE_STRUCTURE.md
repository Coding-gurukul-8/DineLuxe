# 🍽️ Restaurant OS — Final Project File Structure
...
│   ├── export-report/
│   │   └── index.ts                           ← Edge Fn: async CSV/Excel export + S3 upload
│   └── overdue-alert/
...
| 🔮 | Phase 2 — build after MVP launch |
| 💸 | Payment-related — TBD gateway (add Razorpay/Stripe later) |
...
| Section | Files |
|---------|-------|
| Frontend (`app/` pages) | ~85 |
| Frontend (`components/`) | ~65 |
| Frontend (`hooks/` `store/` `lib/`) | ~25 |
| Backend (`src/`) | ~90 |
| Shared | ~8 |
| Supabase | ~12 |
| Docs + Config | ~20 |
